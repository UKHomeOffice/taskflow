...
module.exports = () => {
  ...
  router.put('/', (req, res, next) => {
    return Promise.resolve()
      ...
      .then(result => {
        ...
        return req.task.update(data, { user: req.user, payload: req.body, _id: req._id });
      })
      .then(() => {
  ...
  router.put('/status', (req, res, next) => {
    return Promise.resolve()
      .then(() => {
        return req.task.status(req.body.status, { user: req.user, payload: req.body, _id: req._id });
      })
      .then(() => {
        return Task.find(req.task.id, req.transaction);
      })
